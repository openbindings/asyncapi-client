...
func resolveArtifactReferences(ctx context.Context, client *http.Client, location string, data []byte) ([]byte, error) {
	...
	// silently erase the evidence the refusal is about.
	if err := ValidateReferenceAdmission(root); err != nil {
		return nil, err
	}
...
// demanding an object root. The Avro correspondence admits non-object
...
func avroSchemaChild(owner map[string]any, key string) bool {
	...
	return ok && isAvroSchemaFormat(format)
}
...
// scopedDocument applies JSON Schema's $id base/resource rule to descendant
// references. AsyncAPI message payloads can embed JSON Schema resources, so
// resolving every relative reference against only the retrieval URI is not
// artifact-faithful.
...
// name is a string, both legal Avro schema forms (the §9.2 named
// correspondence).
...
func normalizeResolvedReferenceUnions(root map[string]any) {
	...
	normalizeChannel := func(channelName string, raw any) {
		...
		for index, rawServer := range rawServers {
			...
			name := uniqueReferenceKey(servers, "__asyncapi_client_external_"+sanitizeReferenceKey(channelName)+"_server_"+strconv.Itoa(index))
			servers[name] = server
	...
	for _, operationName := range sortedReferenceKeys(operations) {
		...
		if inline, ok := inlineReferenceObject(operation["channel"]); ok {
			name := uniqueReferenceKey(channels, "__asyncapi_client_external_"+sanitizeReferenceKey(operationName)+"_channel")
		...
		if reply, ok := operation["reply"].(map[string]any); ok {
			if inline, ok := inlineReferenceObject(reply["channel"]); ok {
				name := uniqueReferenceKey(channels, "__asyncapi_client_external_"+sanitizeReferenceKey(operationName)+"_reply_channel")
				channels[name] = inline
				normalizeChannel(name, inline)
...
func normalizeInlineMessages(root map[string]any, prefix string, owner map[string]any, field string) {
	...
	for index, raw := range values {
		...
		name := uniqueReferenceKey(messages, "__asyncapi_client_external_"+sanitizeReferenceKey(prefix)+"_"+strconv.Itoa(index))
