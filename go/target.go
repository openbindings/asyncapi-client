...
// asyncapi@2 §9.2 (ASYNC-P-04): the effective server set and
...
func resolveTarget(doc *document, ch *channel, bindCtx map[string]any) (resolvedTarget, error) {
	...
	if def == nil {
		...
		// target (ASYNC-P-04, ruled 2026-08-13): reachability is consumer
...
func resolveServerConfig(raw any, candidates []namedServer, def *namedServer) (resolvedTarget, error) {
	...
	if member == nil {
		if len(candidates) == 0 {
			...
			// url form alone carries the whole target (ASYNC-P-04).
			if hasVars {
...
func evaluatePayloadLocation(location string, payload any, name string) (string, error) {
	...
	if pointer != "" {
		...
		for _, raw := range strings.Split(strings.TrimPrefix(pointer, "/"), "/") {
			...
			object, ok := toStringAnyMap(current)
			if !ok {
